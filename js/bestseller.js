      ...
      const updateBox = (id, course, rankLabel) => {
        ...
        if (el && course) {
          ...
          el.innerHTML = `
                    ...
                    `;
          if (el.previousElementSibling) el.previousElementSibling.innerText = rankLabel;
        }
      };
      if (top.length > 0) updateBox("bestSellerAmount", top[0], "1ST BEST SELLER");
      if (top.length > 1) updateBox("secondSellerAmount", top[1], "2ND BEST SELLER");
      if (top.length > 2) updateBox("thirdSellerAmount", top[2], "3RD BEST SELLER");
      if (data.worstSeller) {
        ...
        if (el) {
          ...
          if (el.previousElementSibling) el.previousElementSibling.innerText = "LOWEST SELLER";
        }
      }
...
function renderBestsellerTable(records) {
  ...
  const sortedRecords = [...records].sort((a, b) => productStats[b.productName] - productStats[a.productName]);
...
window.applyFilters = function () {
  ...
  if (searchTerm) {
    filtered = filtered.filter((item) => (item.productName || "").toLowerCase().includes(searchTerm));
  }
...
async function exportToCSV() {
  ...
  if (result.isConfirmed) {
    try {
      ...
      link.setAttribute("download", `BR30_Elite_Best_Sellers_${Date.now()}.csv`);
